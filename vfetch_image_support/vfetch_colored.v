// vfetch_colored.v - vfetch with colored ASCII art support
...
fn read_config() Config {
	...
	default_config := Config{
		...
		show_colors: true
		show_ascii_art: true
		logo_color: '35'  // Default magenta
		username_color: '36'  // Default cyan
		info_color: '37'  // Default white
		highlight_color: '33'  // Default yellow
	}
...
// Apply color to text
fn apply_color(text string, color_code string) string {
	if color_code == '' || color_code == 'none' {
		return text
	}
...
// Print logo based on user preference with colors
fn print_logo(logo_choice string, config_file string, config Config) {
	mut logo := ''
	...
	// Apply color to logo if enabled
	if config.show_colors && config.logo_color != 'none' {
		logo = apply_color(logo, config.logo_color)
	}
	
	println(logo)
}

// Execute shell command to display image using Nim handler
fn display_image(image_path string, display_mode string) {
	// Call the Nim image handler program
	if image_path != '' {
		// In a real implementation, we would execute the Nim image handler
...
fn main() {
	...
	print_logo(logo_choice, final_logo_file, config)
	
	// Print username with color
	mut username_line := '${info.username}@${info.os_name}'
	if config.show_colors && config.username_color != 'none' {
		username_line = apply_color(username_line, config.username_color)
	}
	println(username_line)

	// Print separator with color
	mut separator := '----------------'
	if config.show_colors && config.highlight_color != 'none' {
		separator = apply_color(separator, config.highlight_color)
	}
	println(separator)
	...
	for i := 0; i < info_items.len; i++ {
		mut item := info_items[i]
		// Split the item to color the label separately
		parts := item.split(': ')
		if parts.len >= 2 && config.show_colors {
			label := parts[0] + ':'
			value := parts[1..].join(': ')  // Join in case value contains colons
			colored_label := if config.highlight_color != 'none' { apply_color(label, config.highlight_color) } else { label }
			colored_value := if config.info_color != 'none' { apply_color(value, config.info_color) } else { value }
			item = '${colored_label} ${colored_value}'
		}
		println(item)
	}
}